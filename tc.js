var tlist = {
  ...
  21: ["劳动", "2028-05-01", "春风得意花开好，勤劳奋斗梦远行。双手创造幸福路，汗水凝聚富贵情。"],
  22: ["端午", "2028-05-28", "粽香四溢龙舟行，五月五日节传情。祭屈原人心志，平安康健乐满盈。"],
  23: ["国庆", "2028-10-01", "祖国河山美如画，亿万人民庆华诞。山河壮丽同共祝，国泰民安岁月长。"],
  24: ["中秋", "2028-10-03", "银盘高悬夜如银，千里共婵娟。望月思亲心常在，团圆欢乐又一年。"]
};
let tnow = new Date();
let tnowf =
...
function now() {
  const len = Object.getOwnPropertyNames(tlist).length;
  for (var i = 1; i <= len; i++) {
...
function safeIndex(i) {
  const keys = Object.getOwnPropertyNames(tlist);
...
const calendar = {
    ...
    solarTerm: ["\u5c0f\u5bd2", "\u5927\u5bd2", "\u7acb\u6625", "\u96e8\u6c34", "\u60ca\u86f0", "\u6625\u5206", "\u6e05\u660e", "\u8c37\u96e8", "\u7acb\u590f", "\u5c0f\u6ee1", "\u8292\u79cd", "\u590f\u81f3", "\u5c0f\u6691", "\u5927\u6691", "\u7acb\u79cb", "\u5904\u590f", "\u767d\u9732", "\u79cb\u5206", "\u5bd2\u9732", "\u971c\u964d", "\u7acb\u51ac", "\u5927\u51ac", "\u5c0f\u51ac", "\u5927\u51ac"],
...
const len = Object.getOwnPropertyNames(tlist).length;
const next1 = safeIndex(nowlist + 1);
const next2 = safeIndex(nowlist + 2);

$done({
title: title_random(Number(tnumcount(nowlist))),
icon: icon_now(Number(tnumcount(nowlist))),
content: tlist[nowlist][0] + ":" + today(tnumcount(nowlist)) + "," + tlist[next1][0] + ":" + tnumcount(next1) + "天," + tlist[next2][0] + ":" + tnumcount(next2)
})
